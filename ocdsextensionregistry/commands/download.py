...
class Command(BaseCommand):
    ...
    def add_arguments(self):
        self.add_argument("output_directory", help="the directory in which to write the output")
        self.add_argument(
            "versions", nargs="*", help="the versions of extensions to download (e.g. 'bids' or 'lots==master')"
        )
        self.add_argument(
            "--overwrite",
            choices=["any", "none", "live"],
            help="overwrite any downloaded versions (any), no downloaded versions (none), "
            "or only live versions (live) like the master branch",
        )
    ...
    def handle(self):
        ...
        for version in self.versions():
            if not version.download_url:
                logger.warning("Not downloading %s==%s (no Download URL)", version.id, version.version)
                continue
