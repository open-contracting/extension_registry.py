...
def test_remote_nonexistent():
    ...
    assert (
        str(excinfo.value)
        == "404 Client Error: Not Found for url: https://raw.githubusercontent.com/open-contracting-extensions/ocds_location_extension/v1.1.3/nonexistent"
    )
...
def test_remote_codelists_only_download_url():
    args = dict.fromkeys(["Id", "Date", "Version", "Base URL", "Download URL"])
    args["Download URL"] = (
        "https://api.github.com/repos/contratacionesabiertas/ocds_publicNotices_extension/zipball/master"
    )
...
def test_codelists():
    ...
    assert len(result) == 2
    assert result["locationGazetteers.csv"].fieldnames == [
        "Category",
        "Code",
        "Title",
        "Description",
...
def test_codelists_without_metadata():
    ...
    assert len(result) == 1
    assert result["locationGazeteers.csv"].fieldnames == [
        "Category",
        "Code",
        "Title",
        "Description",
...
def test_codelists_with_cr_newlines():
    ...
    assert len(result) == 2
    assert result["bidStatistics.csv"].fieldnames == [
        "Category",
        "Code",
        "Title",
        "Description",
